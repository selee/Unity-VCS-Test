...
public class ReadyMatch : MonoBehaviour
{
	...
	//data to send over the http request
	String requestData;
	...
	//lets the update know that the http requests have completed
	bool registerFinish = false;
	bool updateFinish = false;
	bool joinFinish = false;
	bool authorizeFinish = false;
	...
	private void RegistrationStart ()
	{
		...
		requestData = JSON.Encode (BuildSession ());
		
		SendRegisterWebRequest ();
	}

	/// <summary>
	/// Send starts the register web request 
	/// </summary>
	void SendRegisterWebRequest ()
	{
		
		//sets the web request
		HttpWebRequest webRequest = (HttpWebRequest)WebRequest.Create (masterServerURL + "/session");
		
		webRequest.ContentType = "application/json";
		webRequest.Method = "POST";
		webRequest.Headers[HttpRequestHeader.Authorization] = authToken;
		...
		try {
			...
			//Attach the json data and other crap
			using (Stream dataStream = webRequest.GetRequestStream ()) {
				dataStream.Write (byteArray, 0, byteArray.Length);
				dataStream.Close ();
			}
			//begin the request
			webRequest.BeginGetResponse (new AsyncCallback (GetRegisterWebResult), webRequest);
	...
	/// <summary>
	///function that is called asynchronously when the http request finishes. 
	/// </summary>
	/// <param name="result">
	/// A <see cref="IAsyncResult"/>
	/// </param>
	private void GetRegisterWebResult (IAsyncResult result)
	{
		//if the search has been cancelled before the result returns, sets the process back to the beginning.
		if (searchCancelled) {
			searchCancelled = false;
		...
		try {
			...
			String res = HTTPResponseToString (response);
	...
	/// <summary>
	/// begins sending the join request to the server. 
	/// </summary>
	void SendJoinWebRequest ()
	{
		
		//sets the web request
		HttpWebRequest webRequest = (HttpWebRequest)WebRequest.Create (masterServerURL + "/match");
		...
		try {
			...
			//begin the request
			webRequest.BeginGetResponse (new AsyncCallback (GetJoinWebResult), webRequest);
	...
	/// A <see cref="IAsyncResult"/>
	/// </param>
	private void GetJoinWebResult (IAsyncResult result)
	{
	...
	void SendUnregisterWebRequest ()
	{
		...
		//sets the web request
		HttpWebRequest webRequest = (HttpWebRequest)WebRequest.Create (masterServerURL + "/session/" + sessionID);
		...
		try {
			...
			webRequest.BeginGetResponse (new AsyncCallback (GetUnregisterWebResult), webRequest);
			...
			//Who cares if deleting the record fails?
	...
	/// A <see cref="IAsyncResult"/>
	/// </param>
	private void GetUnregisterWebResult (IAsyncResult result)
	{
	...
	private void UpdateStart ()
	{
		...
		SendUpdateWebRequest ();
	}

	/// <summary>
	/// Send starts the register web request 
	/// </summary>
	void SendUpdateWebRequest ()
	{
		...
		try {
			...
			//begin the request
			webRequest.BeginGetResponse (new AsyncCallback (GetUpdateWebResult), webRequest);
	...
	/// A <see cref="IAsyncResult"/>
	/// </param>
	private void GetUpdateWebResult (IAsyncResult result)
	{
		//if the search has been cancelled before the result returns, sets the process back to the beginning.
		if (searchCancelled) {
			searchCancelled = false;
		...
		try {
			...
			//in the event of the service going down in the middle of matchmaking, disconnect.
			if (players < minPlayers) {
				serviceUnavailable = true;
				Disconnect ();
			}
	...
	private void AuthorizeStart ()
	{
		...
		//json encode the data.
		requestData = JSON.Encode (auth);
		
		SendAuthorizeWebRequest ();
	}

	/// <summary>
	/// Send starts the register web request 
	/// </summary>
	void SendAuthorizeWebRequest ()
	{
		
		//sets the web request
		HttpWebRequest webRequest = (HttpWebRequest)WebRequest.Create (authServerURL + "/authorize");
		...
		try {
			...
			//begin the request
			webRequest.BeginGetResponse (new AsyncCallback (GetAuthorizeWebResult), webRequest);
	...
	/// A <see cref="IAsyncResult"/>
	/// </param>
	private void GetAuthorizeWebResult (IAsyncResult result)
	{
