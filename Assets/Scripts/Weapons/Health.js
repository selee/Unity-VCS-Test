...
function OnDamage (amount : float, fromDirection : Vector3) {
	...
	*/
	...
	var stats : Stats = GetComponent.<Stats>();							
	if(stats){
		stats.OnDamageTaken(amount);
	}
	
	if(!networkView || networkView.isMine || Network.peerType == NetworkPeerType.Disconnected)	
		health -= amount;
	...
	if (health <= 0) {
		...
		if(networkView && networkView.isMine && Network.peerType != NetworkPeerType.Disconnected){
			networkView.RPC("Kill",RPCMode.Others);
		}
