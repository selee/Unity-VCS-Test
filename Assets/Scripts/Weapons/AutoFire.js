		...
		if (Time.time > lastFireTime + 1 / frequency) {
			...
			if (hitInfo.transform) {
				...
				if (targetHealth) {
					...
					var stats : Stats = Player.GetComponent.<Stats>();
					
					if(stats){
						stats.OnDamageDealt(damagePerSecond/ frequency);
						var alive = !targetHealth.dead;
						
						targetHealth.OnDamage (damagePerSecond / frequency, -spawnPoint.forward);
					
						if(targetHealth.dead && alive){
							if(targetHealth.gameObject.tag=="Player")
							stats.OnPlayerKill(targetHealth.points);
							else
							stats.OnBotKill(targetHealth.points);
						}
					}else{
