...
private var lastFixed : float = 0;
private var calculatedPosition:Vector3 = Vector3.zero;
...
function FixedUpdate () {
	...
	if(tr.position == calculatedPosition && !((Network.peerType != NetworkPeerType.Client && Network.peerType != NetworkPeerType.Server) || !Player || !Player.GetComponent("NetworkView") || (Player.GetComponent("NetworkView") as NetworkView).isMine)){
		tr.position = tr.position+velocity*Time.deltaTime;
		calculatedPosition = tr.position;
	}else{
		if(!(Time.time-lastFixed))
			return;
		velocity = (tr.position - lastPosition) / (Time.time-lastFixed);
